...
export class TektonPlugin extends BaseCIPlugin {
    ...
    private async verifyRegistryLinkInTab(
        ...
        try {
            // Set up response listener before waiting for load to avoid missing the document response
            const responsePromise = externalPage.waitForResponse(
                (res) => res.request().resourceType() === 'document',
                { timeout: 10000 },
            );
            await externalPage.waitForLoadState('domcontentloaded');
            const response = await responsePromise;
            expect(response.status(), `${tabName}: Registry page should return OK`).toBeLessThan(400);
