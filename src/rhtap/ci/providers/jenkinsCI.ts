import { JenkinsClient } from '../../../api/ci/jenkinsClient';
...
export class JenkinsCI extends BaseCI {
  ...
  // private async applyCredential(folderName: string, git: Git): Promise<void> {
  //     try {
  ...
  //             await this.jenkinsClient.createCredential(folderName, 'GITOPS_CREDENTIALS', `fakeUsername:${gitlab.getToken()}`, 'Username with password');
  ...
  //             await this.jenkinsClient.createCredential(folderName, 'GITOPS_CREDENTIALS', `${bitbucket.getUsername()}:${bitbucket.getAppPassword()}`, 'Username with password');
  //         }else {
  ...
  //             await this.jenkinsClient.createCredential(folderName, 'GITOPS_CREDENTIALS', `fakeUsername:${github.getToken()}` , 'Username with password');
  //         }
