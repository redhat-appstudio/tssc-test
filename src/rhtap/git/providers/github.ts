import { GithubClient } from '../../../api/git/githubClient';
...
export class GithubProvider extends BaseGitProvider {
  ...
  constructor(componentName: string, repoOwner: string, templateType: TemplateType) {
    super(componentName, repoOwner, GitType.GITHUB);
  ...
  public async initialize(): Promise<void> {
    this.template = TemplateFactory.createTemplate(this.templateType);
   ...
   */
  public getTemplateType(): TemplateType | null {
  ...
  public override async createSamplePullRequestOnSourceRepo(): Promise<PullRequest> {
    ...
    const result = await this.githubClient.modifyAndCreatePullRequest(
      this.repoOwner,
      this.sourceRepoName,
      this.repoOwner,
      baseBranch,
   ...
   */
  public async createSampleCommitOnSourceRepo(): Promise<string> {
    // Get contentModifications from the template
    if (!this.template) {
      throw new Error('Template not set for this repository');
    }
...
public async mergePullRequest(pullRequest: PullRequest): Promise<PullRequest> {
  ...
  public async extractApplicationImage(environment: Environment): Promise<string> {
    const filePath = `components/${this.componentName}/overlays/${environment}/deployment-patch.yaml`;
