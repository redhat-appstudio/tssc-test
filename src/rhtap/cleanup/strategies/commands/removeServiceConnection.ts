...
/**
 * Command to remove sessions from Azure
 */
export class RemoveServiceConnection extends BaseCommand {
  ...
  private async removeVarsAndSecrets(): Promise<void> {
    await this.azureCI.deleteServiceEndpoint(this.component.getName());
    await this.azureCI.deleteServiceEndpoint(this.component.getGit().getGitOpsRepoName());
  }
}
