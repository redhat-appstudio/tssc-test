...
export class BitbucketClient extends BaseApiClient {
  ...
  /**
   * Gets the Bitbucket API version information
   * 
   * Retrieves version information about the Bitbucket API instance. This can be useful
   * for debugging, logging, or determining API capabilities.
   * 
   * @returns Promise<string | null> The Bitbucket version string, or null if unable to retrieve
   * 
   * @example
   * ```typescript
   * const client = new BitbucketClient(config);
   ...
   * const version = await client.getVersion();
   * if (version) {
   *   console.log(`Bitbucket version: ${version}`);
   * } else {
   *   console.log('Unable to retrieve Bitbucket version');
   * }
   * ```
   */
  async getVersion(): Promise<string | null> {
    try {
      const response = await this.httpClient.get('/application-properties') as any;
