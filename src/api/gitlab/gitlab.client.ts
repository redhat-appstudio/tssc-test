...
export class GitLabClient extends BaseApiClient {
  ...
  /**
   * Gets the GitLab API version information
   * 
   * Retrieves version information about the GitLab API instance. This can be useful
   ...
   * const version = await client.getVersion();
   * if (version) {
   *   console.log(`GitLab API version: ${version}`);
   * } else {
   *   console.log('Unable to retrieve GitLab API version');
   * }
   * ```
   */
  async getVersion(): Promise<string | null> {
    try {
      // Use the Projects API as a fallback since Version API might not be available
      await this.client.Projects.all({ perPage: 1 });
      return 'GitLab API Available'; // Return a generic version since we can't get the actual version
    } catch {
      return null;
    }
  }
