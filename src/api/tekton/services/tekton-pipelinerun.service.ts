...
export class TektonPipelineRunService {
  ...
  ): Array<{ name: string; pipelineTaskName?: string }> {
    const childReferences = (
      pipelineRun.status as unknown as { childReferences?: unknown[] }
    )?.childReferences;
    if (!childReferences) return [];

    return childReferences
      .filter((ref: any): ref is { kind: string; name: string } => ref.kind === 'TaskRun')
      .map(ref => ({
        name: ref.name,
        pipelineTaskName: (ref as any).pipelineTaskName,
      }));
  }
