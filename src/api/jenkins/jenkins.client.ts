...
export class JenkinsClient extends BaseApiClient {
  ...
  /**
   * Gets the Jenkins server version information
   * 
   * Retrieves version information about the Jenkins server instance. This can be useful
   * for debugging, logging, or determining server capabilities.
   * 
   * @returns Promise<string | null> The Jenkins version string, or null if unable to retrieve
   * 
   * @example
   * ```typescript
   * const client = new JenkinsClient(config);
   ...
   * const version = await client.getVersion();
   * if (version) {
   *   console.log(`Jenkins version: ${version}`);
   * } else {
   *   console.log('Unable to retrieve Jenkins version');
   * }
   * ```
   */
