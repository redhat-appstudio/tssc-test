...
The framework automatically detects the test type based on the test plan content:

### Automatic Workflow Detection

- **E2E Tests**: When test plan contains backend tests (e.g., `tssc/full_workflow.test.ts`)
- **UI Tests**: When test plan contains UI tests (e.g., `ui/component.test.ts`)
- **Mixed Tests**: When test plan contains both E2E and UI tests
...
- **`ENABLE_UI_TESTS`** (default: `false`) - Force UI test execution even without UI tests in plan
...
UI_DEPENDS_ON_ALL_E2E=true ENABLE_UI_TESTS=true npm test
```
